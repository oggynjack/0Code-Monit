...
if (config.args["allow-all-chrome-exec"] || process.env.CODE_MONIT_ALLOW_ALL_CHROME_EXEC === "1" || process.env.ZEROCODE_MONIT_ALLOW_ALL_CHROME_EXEC === "1") {
    return true;
}
...
if (process.env.CODE_MONIT_IS_CONTAINER || process.env.ZEROCODE_MONIT_IS_CONTAINER || process.env.ZEROCODE_MONIT_IS_CONTAINER) {
...
throw new Error("This Chromium executable path is not allowed by default. If you are sure this is safe, please set CODE_MONIT_ALLOW_ALL_CHROME_EXEC=1 (or ZEROCODE_MONIT_ALLOW_ALL_CHROME_EXEC=1) to allow it.");
        }
    }
