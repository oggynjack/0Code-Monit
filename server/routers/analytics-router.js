...
router.get("/analytics.js", async (req, res) => {
    ...
    const k = req.query.k || "";
    const js = `(()=>{try{var s=document.currentScript;var k='${(k || "").toString().replace(/'/g, "\\'")}';if(!k&&s){k=s.getAttribute('data-key')||new URL(s.src).searchParams.get('k')||'';}if(!k){console.warn('0Code-Monit: missing data-key');return;}var sid;try{sid=localStorage.getItem('cm_sid');if(!sid){sid=Math.random().toString(36).slice(2,12);localStorage.setItem('cm_sid',sid);}}catch(e){sid=Math.random().toString(36).slice(2,12);}function send(){var u=encodeURIComponent(location.href);var r=encodeURIComponent(document.referrer||'');var t=encodeURIComponent(document.title||'');var url='/collect?k='+encodeURIComponent(k)+'&u='+u+'&r='+r+'&t='+t+'&sid='+encodeURIComponent(sid);if(navigator.sendBeacon){try{navigator.sendBeacon(url);}catch(e){fetch(url,{mode:'no-cors',keepalive:true,credentials:'omit'}).catch(()=>{});}}else{fetch(url,{mode:'no-cors',keepalive:true,credentials:'omit'}).catch(()=>{});} }send();document.addEventListener('visibilitychange',function(){if(document.visibilityState==='hidden'){send();}});window.addEventListener('pagehide',send);}catch(e){}})();`;
    res.end(js);
});
