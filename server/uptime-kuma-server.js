...
class CodeMonitServer {
     ...
     * @returns {CodeMonitServer} Server instance
     */
    static getInstance() {
        if (CodeMonitServer.instance == null) {
            CodeMonitServer.instance = new CodeMonitServer();
        }
        return CodeMonitServer.instance;
    }
     ...
     */
    constructor() {
        // Set axios default user-agent to 0Code-Monit/version
        ...
        // Set Monitor Types
        CodeMonitServer.monitorTypeList["real-browser"] = new RealBrowserMonitorType();
        CodeMonitServer.monitorTypeList["tailscale-ping"] = new TailscalePing();
        CodeMonitServer.monitorTypeList["dns"] = new DnsMonitorType();
        CodeMonitServer.monitorTypeList["mqtt"] = new MqttMonitorType();
        CodeMonitServer.monitorTypeList["smtp"] = new SMTPMonitorType();
        CodeMonitServer.monitorTypeList["group"] = new GroupMonitorType();
        CodeMonitServer.monitorTypeList["snmp"] = new SNMPMonitorType();
    ...
    async startNSCDServices() {
        if (process.env.ZEROCODE_MONIT_IS_CONTAINER) {
            try {
     ...
     */
    getUserAgent() {
        return "0Code-Monit/" + require("../package.json").version;
    }
