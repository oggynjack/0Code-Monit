...
(async () => {
    try {
        ...
        await ssh.connect({
            host: process.env.CODE_MONIT_DEMO_HOST,
            port: process.env.CODE_MONIT_DEMO_PORT,
            username: process.env.CODE_MONIT_DEMO_USERNAME,
            privateKeyPath: process.env.CODE_MONIT_DEMO_PRIVATE_KEY_PATH
        });

        let cwd = process.env.CODE_MONIT_DEMO_CWD;
        let result;
